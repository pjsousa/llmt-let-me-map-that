...
- **Missing Phase 2 section JSX:** The current project-detail-page.tsx includes Phase 2 state variables (editingPhase2, phase2Draft, phase2ValidationError, phase2TextareaRef) and their handlers, but the return JSX is missing the Phase 2 <section> element — the component goes from the Phase 1 closing `</section>` directly to the outer `</div>`. This was supposed to be delivered in E6-S1 (status: review). E7-S1's plan includes adding the Phase 2 section JSX as a prerequisite task.
- **Progressive disclosure 'Expand' vs 'Edit' semantics:** For collapsed filled sections, the plan includes an 'Expand' button that expands the section to show full content without entering edit mode, and an 'Edit' button (shown once expanded) that enters edit mode. For collapsed unfilled sections, a single 'Edit' button both expands and enters edit mode. This avoids forcing the user into edit mode just to view their saved data.
